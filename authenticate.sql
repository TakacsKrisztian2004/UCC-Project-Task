...
--
-- Gép: 127.0.0.1
-- Létrehozás ideje: 2025. Már 15. 00:54
...
--
DROP TRIGGER IF EXISTS `delete_from_userreports`;
DELIMITER $$
CREATE TRIGGER `delete_from_userreports` AFTER DELETE ON `aspnetusers` FOR EACH ROW BEGIN
    DELETE FROM userreports.users WHERE Name = OLD.UserName;
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `insert_into_userreports`;
DELIMITER $$
CREATE TRIGGER `insert_into_userreports` AFTER INSERT ON `aspnetusers` FOR EACH ROW BEGIN
    INSERT INTO userreports.users (ID, Name) VALUES (UUID(), NEW.UserName);
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `update_userreports`;
DELIMITER $$
CREATE TRIGGER `update_userreports` AFTER UPDATE ON `aspnetusers` FOR EACH ROW BEGIN
    UPDATE userreports.users 
